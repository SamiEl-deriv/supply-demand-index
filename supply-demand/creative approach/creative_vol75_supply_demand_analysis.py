...
from creative_supply_demand_index_engine import SupplyDemandIndexEngine
...
def run_analysis(
    ...
    print(f"  mean_reversion_strength: {engine.mean_reversion_strength}")
    print(f"  fractal_dimension: {engine.fractal_dimension}")
    print(f"  memory_length: {engine.memory_length}")
...
def generate_dynamic_index_path(
    ...
    ma_window: int = 900,  # 15 hours with 5min data (900 points)
...
def create_separate_visualizations(
    ...
    fractal_dims = [s["mean_fractal_dimension"] for s in summary_stats]
    fractal_dims = [fractal_dims[i] for i in sorted_indices]
    ...
    # Add reference line for standard Brownian motion
    plt.axhline(
        y=1.5,
        color="black",
        linestyle="--",
        alpha=0.6,
        label="Standard Brownian (H=0.5)",
    )

    # Formatting
    plt.title("Fractal Characteristics by Exposure Level", fontsize=16)
    plt.xlabel("Exposure Scenarios (sorted by amount)", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(
        f"{plots_dir}/5_fractal_characteristics.png", dpi=300, bbox_inches="tight"
    )
    plt.close()

    print(f"Created 5 separate visualization plots in the '{plots_dir}' directory")
...
def main():
    ...
    # Calculate net exposure in 1-minute windows
    exposure_data = calculate_net_exposure(trades_df, window_minutes=1)
    ...
    generate_dynamic_index_path(
        ...
        num_simulations=100,
        random_seed=1505,
        ma_window=60,  # 1 hour with 1min data
    )
