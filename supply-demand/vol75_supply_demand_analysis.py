...
This script analyzes Vol 75 trade data using the original supply-demand index engine
with mathematically rigorous approaches including:
1. Normal CDF-based exposure-to-probability mapping
2. Black-Scholes framework for drift calculation
3. Standard GBM price path generation with dynamic drift
...
from supply_demand_index_engine import SupplyDemandIndexEngine
...
def run_analysis(
    ...
    """
    Run supply-demand index analysis on exposure data using the original engine
    ...
    """
    # Initialize the original engine with parameters optimized for Vol 75
    engine = SupplyDemandIndexEngine(
        sigma=0.75,  # 75% volatility - matching Vol 75 characteristics
        scale=100_000,  # Exposure sensitivity calibrated to Vol 75
        k=0.45,  # Max probability deviation (0.05 to 0.95 range)
        T=1.0 / (365 * 24),  # 1 hour time horizon
        S_0=100_000,  # Starting price (100,000)
        dt=1 / (86_400 * 365),  # 1 second time step
    )

    print("Initialized Supply-Demand Index Engine with parameters:")
    print(f"  sigma: {engine.sigma:.1%}")
    print(f"  scale: {engine.scale}")
    print(f"  k: {engine.k}")
    print(f"  T: {engine.T * 365 * 24:.1f} hours")
    print(f"  S_0: {engine.S_0}")
    print(f"  dt: {engine.dt * 365 * 24 * 3600:.1f} seconds")
    ...
    print(
        f"Generating supply-demand index from {len(exposure_data)} exposure points..."
    )
    ...
    results = engine.process_exposure_data(
        ...
        num_paths_per_exposure=100,  # 100 Monte Carlo paths per exposure
...
def generate_dynamic_index_path(
    ...
    seconds_per_point: int = 300,  # Generate a point every 5 minutes (300 seconds)
) -> None:
    """
    Generate dynamic index paths that respond to time-varying exposure using the original engine
    ...
    seconds_per_point : int
        Number of seconds between each generated point (300 = 5 minutes)
    """
    print(
        f"Generating dynamic index paths that respond to time-varying exposure with {ma_window}-point moving average..."
    )
    print(f"Using random seed base: {random_seed} for dynamic path generation")
    print(f"Generating index points every {seconds_per_point} seconds ({seconds_per_point/60:.1f} minutes)")
    ...
    plt.title(
        ...
        f"({num_simulations} simulations, {ma_window}-point moving average, {seconds_per_point/60:.1f}-minute resolution)",
        fontsize=14,
    )
    ...
    try:
        ...
        print(f"  Saved: {plots_dir}/6_dynamic_index_path.png")
    except Exception as e:
        print(f"  Error saving 6_dynamic_index_path.png: {e}")
    plt.close()

    # Create a second plot showing the exposure data that generated the paths
    plt.figure(figsize=(16, 6))
    plt.plot(exposure_series.values, color="darkgreen", linewidth=1.5)
    ...
    try:
        ...
        print(f"  Saved: {plots_dir}/7_exposure_data.png")
    except Exception as e:
        print(f"  Error saving 7_exposure_data.png: {e}")
    plt.close()

    # Create a combined plot with exposure and index on the same time axis
    create_combined_exposure_index_plot(
        exposure=exposure_series.values, index_path=mean_path, plots_dir=plots_dir
    )
...
def create_combined_exposure_index_plot(
    ...
    try:
        ...
        print(f"  Saved: {plots_dir}/8_combined_exposure_index.png")
    except Exception as e:
        print(f"  Error saving 8_combined_exposure_index.png: {e}")
    plt.close()
...
def create_separate_visualizations(
    ...
    plt.plot(
        ...
        color="blue",
        linewidth=2.5,
        label="Theoretical Mapping (Normal CDF)",
    )
    ...
    # Formatting
    plt.title("Exposure-to-Probability Mapping (Normal CDF)", fontsize=16)
    ...
    plt.plot(
        ...
        color="blue",
        linewidth=2.5,
        label="Theoretical Mapping (Black-Scholes)",
    )
    ...
    # Formatting
    plt.title("Probability-to-Drift Mapping (Black-Scholes)", fontsize=16)
    ...
    for i, idx in enumerate(indices):
        ...
        # Formatting
        plt.title(f"GBM Price Paths for Exposure: ${exposure:,.0f}", fontsize=14)
        plt.xlabel("Time Steps", fontsize=12)
...
def main():
    ...
    engine, results = run_analysis(
        exposure_data=exposure_data, sample_size=50, random_seed=1505
    )
