...
class VolatilitySwitchIndex(StochasticProcess):
    def __init__(self, 
                 vol: List[float],
                 drift:List[float],
                 T: List[float],
    ...
    def _initialize_transition_matrix(self, P: Optional[np.array]) -> np.array:
        ...
        assert np.allclose(P.sum(axis=0), 1), "collumn of transition matrix must sum to 1." # axis=0 --> columns ; axis=1 --> rows
        return P
    ...
    def choose_next_regime(self):
        ...
        transition_probs = self.__P.transpose()[self.__current_regime]
    ...
    def new_return(self):
        ...
        self.__regimes.append(self.__current_regime)
