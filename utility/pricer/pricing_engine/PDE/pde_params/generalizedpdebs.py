...
class generalizedPDEBlackScholes(PDEConvectionDiffusion):
    """
    A class containing the various parameters for the generalized Black-Scholes PDE with continuous dividends. Rewriting it in the necessary form:

    V_t = - (r-q) * S * V_S - local_vol(t,S)^2 * S^2 * V_{SS} + r*V
    ...
    def __init__(self, Option : VanillaOption, Current_Spot : float,  N_x : int, N_t : int, American : bool, FDMclass : Type[FiniteDifference], mode="fast"):
        """
        Parameters
        ----------
        ...
        if self.option.riskFreeRate(self.option.expiry) > self.option.volatility(self.option.expiry, self.option.strike):
            warnings.warn("Volatility exceeds risk-free rate. May result in solution diverging")

        # Initialize boundary information
        PayOffId = self.option.payOff.__class__.__name__ # get the class of payoff in string format 
        self._Boundary = BoundaryFactory.CreateBoundary(PayOffId, Option=self.option, Current_Spot=Current_Spot)
    ...
    def coefficient_convection(self, x : float, t : float) -> float:
        """
        The convection coefficient (a = -(r-q)*S) of the generalized BS PDE
        ...
        """
        return -(self.option.riskFreeRate(self.option.expiry) - self.option.dividendRate(self.option.expiry)) * x
        
    def coefficient_diffusion(self, x : float, t : float) -> float:
        """
        The diffusion coefficient (a = -0.5 * local_vol(t,S)^2 * S^2) of the geenralized BS PDE
        ...
        """
        return -0.5*(self.option.volatility(t, x) * x)**2
    ...
    def coefficient_zero(self, x : float, t : float) -> float:
        """
        The zero coefficient (c = r) of the generalized BS PDE
    ...
    def get_price(self, Spot : float, Tenor : float) -> float:
        ...
        if Tenor <= max_tenor:
            ...
            raise ValueError(f"Tenor is out of range: min_tenor = 0, max_tenor = {max_tenor}")
