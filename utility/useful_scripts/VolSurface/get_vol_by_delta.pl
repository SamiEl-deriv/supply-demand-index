...
my @buffer = ("underlying,vol,delta,start_date,end_date,interest_rate,dividend_rate,spot,premium_adj\n");

# any historical data, change starting date here
my @from_raw = ("2022-09-06 10:00:00","2022-09-01 10:00:00","2022-07-28 10:00:00","2022-06-04 10:00:00");
...
my @underlying_names = ("frxUSDJPY","frxEURUSD");
...
foreach my $from_date (@from) {
    ...
    foreach my $underlying_name (@underlying_names) {
        ...
        foreach my $to_date (@to) {
            ...
            for (my $delta = $step_size; $delta <= min(100,100*exp(-$q * $t)); $delta += $step_size) {
                my $vol = $volsurface->get_volatility({delta => $delta, from => $from_date, to => $to_date});

                # append_file($output_file, "$underlying_name,$vol,$strike," . $from_date->epoch . "," . $to_date->epoch . ",$r,$q,$spot,$premium_adjusted\n");
                my $line = "$underlying_name,$vol,$delta," . $from_date->epoch . "," . $to_date->epoch . ",$r,$q,$spot,$premium_adjusted\n";
                push(@buffer, $line); 
            }
