...
def accu_tick_size_barrier_builder(loss_probs=default_loss_probs) -> None:
    ...
    def is_CB(symbol):
        return 1 if ('CRASH' in symbol or 'BOOM' in symbol) else 0 

    for symbol in symbols.keys():
        if is_vol(symbol) or is_CB(symbol):
    ...
    def get_CB_tick_size_barrier(symbol, loss_prob):
        def crash(x, *kwargs):
        ...
        if 'CRASH' in symbol:
            return fsolve(crash, 0, args = (loss_prob, re.search(r'\d+', symbol).group()))[0]
        elif 'BOOM' in symbol:
            return fsolve(boom, 0, args = (loss_prob, re.search(r'\d+', symbol).group()))[0]
    ...
    for growth_rate in loss_probs.keys():
        ...
        for symbol in symbols.keys():
            if is_vol(symbol):
                ...
                symbols[symbol]['accu']['barrier'][growth_rate] = get_CB_tick_size_barrier (symbol, loss_prob)
        ...
        for item in self.messages:
            if 'msg_type' in item and item['msg_type'] == 'proposal' and 'error' not in item and 'ACCU' in item['echo_req']['contract_type']:
                ...
                if not item['mv']['barrier_check'] and show:
                    display(item)
        self.stats_barrier = dict_to_df(self.stats['barrier_check'], 'Barrier')   
